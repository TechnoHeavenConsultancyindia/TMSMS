...
using TMSMS.TransferService.HealthChecks;
...
[DependsOn(
    ...
    typeof(TMSMSTransferServiceContractsModule),
...
public class TMSMSTransferServiceModule : AbpModule
{
    ...
    public override async Task OnPreApplicationInitializationAsync(ApplicationInitializationContext context)
    {
        ...
        await scope.ServiceProvider
            .GetRequiredService<TransferServiceRuntimeDatabaseMigrator>()
    ...
    {
        context.Services.AddTransferServiceHealthChecks();
    }
    ...
    private void ConfigureVirtualFileSystem()
    {
        ...
        {
            options.FileSets.AddEmbedded<TMSMSTransferServiceModule>();
        });
    }
    ...
    private void ConfigureSwagger(ServiceConfigurationContext context, IConfiguration configuration)
    {
        if (IsSwaggerEnabled(configuration))
        {
            context.Services.AddAbpSwaggerGenWithOAuth(
                ...
                {
                    {"TransferService", "TransferService Service API"}
                },
                options =>
                {
                    options.SwaggerDoc("v1", new OpenApiInfo { Title = "TransferService API", Version = "v1" });
    ...
    private void ConfigureDatabase(ServiceConfigurationContext context)
    {
        ...
        context.Services.AddAbpDbContext<TransferServiceDbContext>(options =>
        {
            ...
            options.AddRepository<TransferType, TransferServices.EfCoreTransferTypeRepository>();
        ...
        Configure<AbpDbContextOptions>(options =>
        {
            ...
            options.Configure<TransferServiceDbContext>(c =>
            {
                c.UseSqlServer(b =>
                {
    ...
    private void ConfigureDistributedEventBus()
    {
        Configure<AbpDistributedEventBusOptions>(options =>
        {
            ...
            {
                config.UseDbContext<TransferServiceDbContext>();
            });
    ...
    {
        context.Services.AddAutoMapperObjectMapper<TMSMSTransferServiceModule>();

        Configure<AbpAutoMapperOptions>(options =>
        {
            options.AddMaps<TMSMSTransferServiceModule>(validate: true);
        });
    }
    ...
    private void ConfigureAutoControllers()
    {
        Configure<AbpAspNetCoreMvcOptions>(options =>
        {
            options
                ...
                .Create(typeof(TMSMSTransferServiceModule).Assembly, opts =>
                {
                    opts.RemoteServiceName = TransferRemoteServiceConsts.RemoteServiceName;
                    opts.RootPath = TransferRemoteServiceConsts.ModuleName;
                });
        });
    }
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "TransferService API");
        options.OAuthClientId(configuration["AuthServer:SwaggerClientId"]);
        options.OAuthScopes("TransferService");
    }
