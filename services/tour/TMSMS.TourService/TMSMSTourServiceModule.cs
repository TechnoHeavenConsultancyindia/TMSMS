...
using TMSMS.TourService.HealthChecks;
...
[DependsOn(
    ...
    typeof(TMSMSTourServiceContractsModule),
...
public class TMSMSTourServiceModule : AbpModule
{
    ...
    public override async Task OnPreApplicationInitializationAsync(ApplicationInitializationContext context)
    {
        ...
        await scope.ServiceProvider
            .GetRequiredService<TourServiceRuntimeDatabaseMigrator>()
    ...
    {
        context.Services.AddTourServiceHealthChecks();
    }
    ...
    private void ConfigureVirtualFileSystem()
    {
        ...
        {
            options.FileSets.AddEmbedded<TMSMSTourServiceModule>();
        });
    }
    ...
    private void ConfigureSwagger(ServiceConfigurationContext context, IConfiguration configuration)
    {
        if (IsSwaggerEnabled(configuration))
        {
            context.Services.AddAbpSwaggerGenWithOAuth(
                ...
                {
                    {"TourService", "TourService Service API"}
                },
                options =>
                {
                    options.SwaggerDoc("v1", new OpenApiInfo {Title = "TourService API", Version = "v1"});
    ...
    private void ConfigureDatabase(ServiceConfigurationContext context)
    {
        ...
        context.Services.AddAbpDbContext<TourServiceDbContext>(options =>
        {
        ...
        Configure<AbpDbContextOptions>(options =>
        {
            ...
            options.Configure<TourServiceDbContext>(c =>
            {
                c.UseSqlServer(b =>
                {
    ...
    private void ConfigureDistributedEventBus()
    {
        Configure<AbpDistributedEventBusOptions>(options =>
        {
            ...
            {
                config.UseDbContext<TourServiceDbContext>();
            });
    ...
    {
        context.Services.AddAutoMapperObjectMapper<TMSMSTourServiceModule>();

        Configure<AbpAutoMapperOptions>(options =>
        {
            options.AddMaps<TMSMSTourServiceModule>(validate: true);
        });
    }
    ...
    private void ConfigureAutoControllers()
    {
        Configure<AbpAspNetCoreMvcOptions>(options =>
        {
            options
                ...
                .Create(typeof(TMSMSTourServiceModule).Assembly, opts =>
                {
                    opts.RemoteServiceName = TourRemoteServiceConsts.RemoteServiceName;
                    opts.RootPath = TourRemoteServiceConsts.ModuleName;
                });
        });
    }
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "TourService API");
        options.OAuthClientId(configuration["AuthServer:SwaggerClientId"]);
        options.OAuthScopes("TourService");
    }
