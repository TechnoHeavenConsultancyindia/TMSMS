...
+   metrics_path: 'metrics'
+   static_configs:
+   - targets: ['host.docker.internal:44316']
```
...
```bash
abp generate-proxy -t csharp -url http://localhost:44316/ -m tour --without-contracts
```
...
```bash
abp generate-proxy -t ng -url http://localhost:44316/ -m tour
```