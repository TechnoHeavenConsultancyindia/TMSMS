...
{
    public abstract class EfCoreRegionRepositoryBase : EfCoreRepository<CommonServiceDbContext, Region, int>
    {
        public EfCoreRegionRepositoryBase(IDbContextProvider<CommonServiceDbContext> dbContextProvider)
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            query = ApplyFilter(query, filterText, locationId, name, fullName, descriptor, iataAirportCode, iataAirportMetroCode, countrySubdivisionCode, latitude, longitude, polygonType, categories, countryCode, tags, statusFlagMin, statusFlagMax);
        ...
        public virtual async Task<List<Region>> GetListAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var query = ApplyFilter((await GetQueryableAsync()), filterText, locationId, name, fullName, descriptor, iataAirportCode, iataAirportMetroCode, countrySubdivisionCode, latitude, longitude, polygonType, categories, countryCode, tags, statusFlagMin, statusFlagMax);
        ...
        public virtual async Task<long> GetCountAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var query = ApplyFilter((await GetDbSetAsync()), filterText, locationId, name, fullName, descriptor, iataAirportCode, iataAirportMetroCode, countrySubdivisionCode, latitude, longitude, polygonType, categories, countryCode, tags, statusFlagMin, statusFlagMax);
        ...
        {
            return query
                    .WhereIf(!string.IsNullOrWhiteSpace(filterText), e => e.LocationId!.Contains(filterText!) || e.Name!.Contains(filterText!) || e.FullName!.Contains(filterText!) || e.Descriptor!.Contains(filterText!) || e.IataAirportCode!.Contains(filterText!) || e.IataAirportMetroCode!.Contains(filterText!) || e.CountrySubdivisionCode!.Contains(filterText!) || e.Latitude!.Contains(filterText!) || e.Longitude!.Contains(filterText!) || e.PolygonType!.Contains(filterText!) || e.Categories!.Contains(filterText!) || e.CountryCode!.Contains(filterText!) || e.Tags!.Contains(filterText!))
