...
using TMSMS.CommonService.HealthChecks;
...
[DependsOn(
    ...
    typeof(TMSMSCommonServiceContractsModule),
...
public class TMSMSCommonServiceModule : AbpModule
{
    ...
    public override async Task OnPreApplicationInitializationAsync(ApplicationInitializationContext context)
    {
        ...
        await scope.ServiceProvider
            .GetRequiredService<CommonServiceRuntimeDatabaseMigrator>()
    ...
    {
        context.Services.AddCommonServiceHealthChecks();
    }
    ...
    private void ConfigureVirtualFileSystem()
    {
        ...
        {
            options.FileSets.AddEmbedded<TMSMSCommonServiceModule>();
        });
    }
    ...
    private void ConfigureSwagger(ServiceConfigurationContext context, IConfiguration configuration)
    {
        if (IsSwaggerEnabled(configuration))
        {
            context.Services.AddAbpSwaggerGenWithOAuth(
                ...
                {
                    {"CommonService", "CommonService Service API"}
                },
                options =>
                {
                    options.SwaggerDoc("v1", new OpenApiInfo { Title = "CommonService API", Version = "v1" });
    ...
    private void ConfigureDatabase(ServiceConfigurationContext context)
    {
        ...
        context.Services.AddAbpDbContext<CommonServiceDbContext>(options =>
        {
            ...
            options.AddRepository<Country, CommonServices.EfCoreCountryRepository>();
            ...
            options.AddRepository<ServiceType, CommonServices.EfCoreServiceTypeRepository>();
        ...
        Configure<AbpDbContextOptions>(options =>
        {
            ...
            options.Configure<CommonServiceDbContext>(c =>
            {
                c.UseSqlServer(b =>
                {
    ...
    private void ConfigureDistributedEventBus()
    {
        Configure<AbpDistributedEventBusOptions>(options =>
        {
            ...
            {
                config.UseDbContext<CommonServiceDbContext>();
            });
    ...
    {
        context.Services.AddAutoMapperObjectMapper<TMSMSCommonServiceModule>();

        Configure<AbpAutoMapperOptions>(options =>
        {
            options.AddMaps<TMSMSCommonServiceModule>(validate: true);
        });
    }
    ...
    private void ConfigureAutoControllers()
    {
        Configure<AbpAspNetCoreMvcOptions>(options =>
        {
            options
                ...
                .Create(typeof(TMSMSCommonServiceModule).Assembly, opts =>
                {
                    opts.RemoteServiceName = CommonRemoteServiceConsts.RemoteServiceName;
                    opts.RootPath = CommonRemoteServiceConsts.ModuleName;
                });
        });
    }
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "CommonService API");
        options.OAuthClientId(configuration["AuthServer:SwaggerClientId"]);
        options.OAuthScopes("CommonService");
    }
