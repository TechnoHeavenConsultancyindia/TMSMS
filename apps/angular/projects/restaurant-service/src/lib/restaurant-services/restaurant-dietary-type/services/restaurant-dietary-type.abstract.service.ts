...
import { RestaurantDietaryTypeService } from '../../../proxy/restaurant-service/restaurant-services/restaurant-dietary-type.service';

export abstract class AbstractRestaurantDietaryTypeViewService {
  protected readonly proxyService = inject(RestaurantDietaryTypeService);
  ...
  data: PagedResultDto<RestaurantDietaryTypeDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<RestaurantDietaryTypeDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetRestaurantDietaryTypesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<RestaurantDietaryTypeDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetRestaurantDietaryTypesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'RestaurantDietaryType.xlsx');
      });
  }
}
