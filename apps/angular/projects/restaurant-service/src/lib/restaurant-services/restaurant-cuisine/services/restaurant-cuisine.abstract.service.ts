...
import { RestaurantCuisineService } from '../../../proxy/restaurant-service/restaurant-services/restaurant-cuisine.service';

export abstract class AbstractRestaurantCuisineViewService {
  protected readonly proxyService = inject(RestaurantCuisineService);
  ...
  data: PagedResultDto<RestaurantCuisineDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<RestaurantCuisineDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetRestaurantCuisinesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<RestaurantCuisineDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetRestaurantCuisinesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'RestaurantCuisine.xlsx');
      });
  }
}
