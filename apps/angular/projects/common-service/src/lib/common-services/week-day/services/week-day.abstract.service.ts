...
export abstract class AbstractWeekDayViewService {
  protected readonly proxyService = inject(WeekDayService);
  ...
  data: PagedResultDto<WeekDayDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<WeekDayDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetWeekDaysInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<WeekDayDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetWeekDaysInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'WeekDay.xlsx');
      });
  }
}
