...
export abstract class AbstractPromoCodeUsageTrackingViewService {
  protected readonly proxyService = inject(PromoCodeUsageTrackingService);
  ...
  data: PagedResultDto<PromoCodeUsageTrackingWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<PromoCodeUsageTrackingWithNavigationPropertiesDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetPromoCodeUsageTrackingsInput;
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ promoCodeUsageTracking: { id } }) => id);
  ...
  delete(record: PromoCodeUsageTrackingWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.promoCodeUsageTracking.id)),
      )
  ...
  hookToQuery() {
    ...
    const setData = (
      list: PagedResultDto<PromoCodeUsageTrackingWithNavigationPropertiesDto>,
    ) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetPromoCodeUsageTrackingsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'PromoCodeUsageTracking.xlsx');
      });
  }
}
