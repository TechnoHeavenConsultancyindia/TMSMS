...
export abstract class AbstractPromoCodeMasterViewService {
  protected readonly proxyService = inject(PromoCodeMasterService);
  protected readonly confirmationService = inject(ConfirmationService);
  protected readonly list = inject(ListService);
  protected readonly abpWindowService = inject(AbpWindowService);

  public readonly getWithNavigationProperties =
    this.proxyService.getWithNavigationProperties;

  isExportToExcelBusy = false;

  data: PagedResultDto<PromoCodeMasterWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<PromoCodeMasterWithNavigationPropertiesDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetPromoCodeMastersInput;
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ promoCodeMaster: { id } }) => id);
  ...
  delete(record: PromoCodeMasterWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.promoCodeMaster.id)),
      )
  ...
  hookToQuery() {
    ...
    const setData = (
      list: PagedResultDto<PromoCodeMasterWithNavigationPropertiesDto>,
    ) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetPromoCodeMastersInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'PromoCodeMaster.xlsx');
      });
  }
}
