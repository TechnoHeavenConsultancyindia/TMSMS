...
export abstract class AbstractSupplierMasterViewService {
  protected readonly proxyService = inject(SupplierMasterService);
  ...
  data: PagedResultDto<SupplierMasterWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<SupplierMasterWithNavigationPropertiesDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetSupplierMastersInput;
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ supplierMaster: { id } }) => id);
  ...
  delete(record: SupplierMasterWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.supplierMaster.id)),
      )
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<SupplierMasterWithNavigationPropertiesDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetSupplierMastersInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'SupplierMaster.xlsx');
      });
  }
}
