...
export abstract class AbstractCityViewService {
  protected readonly proxyService = inject(CityService);
  ...
  data: PagedResultDto<CityDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<CityDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetCitiesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<CityDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetCitiesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'City.xlsx');
      });
  }
}
