...
import { ServiceTypeService } from '../../../proxy/common-service/common-services/service-type.service';

export abstract class AbstractServiceTypeViewService {
  protected readonly proxyService = inject(ServiceTypeService);
  ...
  data: PagedResultDto<ServiceTypeDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<ServiceTypeDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetServiceTypesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<ServiceTypeDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetServiceTypesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'ServiceType.xlsx');
      });
  }
}
