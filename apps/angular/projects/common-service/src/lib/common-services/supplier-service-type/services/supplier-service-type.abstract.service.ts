...
import { SupplierServiceTypeService } from '../../../proxy/common-service/common-services/supplier-service-type.service';

export abstract class AbstractSupplierServiceTypeViewService {
  protected readonly proxyService = inject(SupplierServiceTypeService);
  ...
  data: PagedResultDto<SupplierServiceTypeDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<SupplierServiceTypeDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetSupplierServiceTypesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<SupplierServiceTypeDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetSupplierServiceTypesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'SupplierServiceType.xlsx');
      });
  }
}
