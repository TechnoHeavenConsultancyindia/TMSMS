...
import { VisaDiscountCategoryService } from '../../../proxy/visa-service/visa-services/visa-discount-category.service';

export abstract class AbstractVisaDiscountCategoryViewService {
  protected readonly proxyService = inject(VisaDiscountCategoryService);
  ...
  data: PagedResultDto<VisaDiscountCategoryDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<VisaDiscountCategoryDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetVisaDiscountCategoriesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<VisaDiscountCategoryDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetVisaDiscountCategoriesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'VisaDiscountCategory.xlsx');
      });
  }
}
