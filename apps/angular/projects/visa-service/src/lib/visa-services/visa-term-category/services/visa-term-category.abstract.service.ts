...
import { VisaTermCategoryService } from '../../../proxy/visa-service/visa-services/visa-term-category.service';

export abstract class AbstractVisaTermCategoryViewService {
  protected readonly proxyService = inject(VisaTermCategoryService);
  ...
  data: PagedResultDto<VisaTermCategoryDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<VisaTermCategoryDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetVisaTermCategoriesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<VisaTermCategoryDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetVisaTermCategoriesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'VisaTermCategory.xlsx');
      });
  }
}
