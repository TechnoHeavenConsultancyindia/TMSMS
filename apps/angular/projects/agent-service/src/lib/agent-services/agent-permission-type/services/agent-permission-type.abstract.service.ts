...
export abstract class AbstractAgentPermissionTypeViewService {
  protected readonly proxyService = inject(AgentPermissionTypeService);
  ...
  data: PagedResultDto<AgentPermissionTypeDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<AgentPermissionTypeDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetAgentPermissionTypesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<AgentPermissionTypeDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetAgentPermissionTypesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'AgentPermissionType.xlsx');
      });
  }
}
