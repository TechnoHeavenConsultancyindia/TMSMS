...
import type {
  ...
  AgentVoucherTypeDto,
} from '../../../proxy/agent-service/agent-services/models';
import { AgentVoucherTypeService } from '../../../proxy/agent-service/agent-services/agent-voucher-type.service';

export abstract class AbstractAgentVoucherTypeViewService {
  protected readonly proxyService = inject(AgentVoucherTypeService);
  ...
  data: PagedResultDto<AgentVoucherTypeDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<AgentVoucherTypeDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetAgentVoucherTypesInput;
  ...
  delete(record: AgentVoucherTypeDto) {
    this.confirmationService
      .warn('AgentService::DeleteConfirmationMessage', 'AgentService::AreYouSure', {
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<AgentVoucherTypeDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetAgentVoucherTypesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'AgentVoucherType.xlsx');
      });
  }
}
