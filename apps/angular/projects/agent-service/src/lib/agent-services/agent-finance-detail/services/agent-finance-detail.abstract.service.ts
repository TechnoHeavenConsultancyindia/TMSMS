...
export abstract class AbstractAgentFinanceDetailViewService {
  protected readonly proxyService = inject(AgentFinanceDetailService);
  ...
  data: PagedResultDto<AgentFinanceDetailDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<AgentFinanceDetailDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetAgentFinanceDetailsInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<AgentFinanceDetailDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetAgentFinanceDetailsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'AgentFinanceDetail.xlsx');
      });
  }
}
