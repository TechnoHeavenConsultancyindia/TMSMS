...
import { AgentGroupTypeService } from '../../../proxy/agent-service/agent-services/agent-group-type.service';

export abstract class AbstractAgentGroupTypeViewService {
  protected readonly proxyService = inject(AgentGroupTypeService);
  ...
  data: PagedResultDto<AgentGroupTypeDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<AgentGroupTypeDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetAgentGroupTypesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<AgentGroupTypeDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetAgentGroupTypesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'AgentGroupType.xlsx');
      });
  }
}
